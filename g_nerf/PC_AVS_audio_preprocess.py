...
def read_audio(path):
    ...
    for frame in container.decode(audio=0):
        for frame_resampled in resampler.resample(frame):
            fifo.write(frame_resampled)
    for frame_resampled in resampler.resample(None):
        fifo.write(frame_resampled)
    return fifo.read().to_ndarray().squeeze() / 32768
...
if __name__ == '__main__':
    ...
    w = args.path
    audio = AudioConfig(num_frames_per_clip=5, hop_size=160)
    wav = audio.read_audio(w)
    spectrogram = audio.audio_to_spectrogram(wav)
    print(w)
    np.save(w.replace('wav', 'npy'), spectrogram)