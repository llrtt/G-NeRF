...
def log_image_from_w(w, G, name, camera_params):
    img = get_image_from_w(w, G, camera_params)
...
def get_image_from_w(w, G, camera_params):
    if len(w.size()) <= 2:
        w = w.unsqueeze(0)
    with torch.no_grad():
        img = G.synthesis(w, camera_params)['image']
        img = ((img.permute(0, 2, 3, 1) + 1) * 127.5).clamp(0, 255).to(torch.uint8).detach().cpu().numpy()
    return img[0]
