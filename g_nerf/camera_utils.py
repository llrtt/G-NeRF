...
class LookAtPoseSampler:
    ...
    @staticmethod
    def sample_origin(horizontal_mean, vertical_mean, lookat_position, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        h = torch.randn((batch_size, 1), device=device) * horizontal_stddev + horizontal_mean
        v = torch.randn((batch_size, 1), device=device) * vertical_stddev + vertical_mean
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
    ...
    @staticmethod
    def sample_srn(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        ...
        camera_origins[:, 0:1] = radius*torch.sin(phi) * torch.sin(theta)
        camera_origins[:, 1:2] = radius*torch.sin(phi) * torch.cos(theta)
        camera_origins[:, 2:3] = radius*torch.cos(phi)

        forward_vectors = math_utils.normalize_vecs(-camera_origins)
        return create_cam2world_matrix_srn(forward_vectors, camera_origins)    
...
def create_cam2world_matrix_srn(forward_vector, origin):
    """
    Takes in the direction the camera is pointing and the camera origin and returns a cam2world matrix.
    Works on batches of forward_vectors, origins. Assumes y-axis is up and that there is no camera roll.
    """
