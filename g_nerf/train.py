...
def launch_training(c, desc, outdir, dry_run):
    ...
    print(f'Train encoder:       {c.train_en}')
    print(f'Train generator:     {c.train_gen}')
    print(f'Z dim:     {c.G_kwargs.z_dim}')
    print(f'w dim:     {c.G_kwargs.w_dim}')
    print()
...
def init_dataset_kwargs(data, dataset):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name=dataset, path=data, use_labels=True, xflip=False)
        ...
        dataset_kwargs.max_size = 60000 # Be explicit about dataset size.
...
@click.option('--dataset_name', help='dataset name', metavar='[ZIP|DIR]',                      type=str, required=True)
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=1), required=True)
@click.option('--batch',        help='Total batch size', metavar='INT',                         type=click.IntRange(min=1), required=True)
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), required=True)
@click.option('--train_en',     help='Wheter to train encoder', metavar='DIR',                  type=bool, required=True)
@click.option('--train_gen',    help='Wheter to train generator', metavar='DIR',                type=bool, required=True)

# Optional features.
@click.option('--cycle_loss',   help='Wheter to train generator', metavar='DIR',                type=bool, default=False, show_default=True)
@click.option('--gan_depth',    help='Wheter to use geometry-aware disc', metavar='DIR',        type=bool, default=True, show_default=True)
...
@click.option('--resume_gen',   help='Resume from given generator pickle', metavar='[PATH|URL]',type=str)
@click.option('--resume_disc',   help='Resume from given generator pickle', metavar='[PATH|URL]',type=str)
@click.option('--resume_en',    help='Resume from given encoder pickle', metavar='[PATH|URL]',  type=str)
...
@click.option('--glr',          help='G learning rate  [default: varies]', metavar='FLOAT',     type=click.FloatRange(min=0), default=0.001, show_default=True)
@click.option('--dlr',          help='D learning rate', metavar='FLOAT',                        type=click.FloatRange(min=0), default=8e-6, show_default=True)
...
@click.option('--mbstd-group',  help='Minibatch std group size', metavar='INT',                 type=click.IntRange(min=1), default=3, show_default=True)

# Misc settings.
@click.option('--desc',         help='String to include in result dir name', metavar='STR',     type=str)
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',            type=parse_comma_separated_list, default='fid50k_full', show_default=True)
@click.option('--kimg',         help='Total training duration', metavar='KIMG',                 type=click.IntRange(min=1), default=4000, show_default=True)
...
@click.option('--gpc_reg_prob', help='Strength of swapping regularization. None means no generator pose conditioning, i.e. condition with zeros.', metavar='FLOAT',  type=click.FloatRange(min=0), required=False, default=True)
...
@click.option('--z_dim',    help='z_dim.', type=int, default=512, required=False, show_default=True)
@click.option('--w_dim',    help='w_dim.', type=int, default=512, required=False, show_default=True)
@click.option('--update_model',   help='update_model.', type=bool, default=False, required=False, show_default=True)
...
def main(**kwargs):
    ...
    c.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0.9,0.999], eps=1e-8)
    ...
    if opts.dataset_name == 'ffhq':
        train_set = 'training.dataset.FFHQ_GEN_Dataset'
        test_set = 'training.dataset.Test_Dataset'
        sr_module = 'training.superresolution.SuperresolutionHybrid8XDC'
        c.training_set_kwargs = dnnlib.EasyDict(class_name=train_set, path=opts.data, use_labels=True, xflip=False,resolution=512,max_size=60000)
        c.testing_set_kwargs = dnnlib.EasyDict(class_name=test_set, path=opts.data, use_labels=True, xflip=False,resolution=512,max_size=60000)
    if opts.dataset_name == 'afhqv2':
        train_set = 'training.dataset.Afhqv2_Dataset'
        test_set = 'training.dataset.Afhqv2_Test_Dataset'
        sr_module = 'training.superresolution.SuperresolutionHybrid8XDC'
        c.training_set_kwargs = dnnlib.EasyDict(class_name=train_set, path=opts.data, use_labels=True, xflip=False,resolution=512,max_size=4000)
        c.testing_set_kwargs = dnnlib.EasyDict(class_name=test_set, path=opts.data, use_labels=True, xflip=False,resolution=512,max_size=4000)
    if opts.dataset_name == 'shapenet':
        train_set = 'training.dataset.ShapeNet_Dataset'
        test_set = 'training.dataset.ShapeNet_Test_Dataset'
        sr_module = 'training.superresolution.SuperresolutionHybrid2X'
        c.training_set_kwargs = dnnlib.EasyDict(class_name=train_set, path=opts.data, use_labels=True, xflip=False,resolution=128,max_size=100000)
        c.testing_set_kwargs = dnnlib.EasyDict(class_name=test_set, path=opts.data, use_labels=True, xflip=False,resolution=128,max_size=100000)
    ...
    elif opts.dataset_name == 'shapenet':
        rendering_options.update({
            'depth_resolution': 64,
    ...
    c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax, fused_modconv_default='inference_only', w_dim=opts.w_dim)

    c.loss_kwargs.style_mixing_prob = opts.style_mixing_prob

    # Resume.
    if opts.resume_gen is not None:
        c.resume_gen = opts.resume_gen
        c.ema_rampup = None # Disable EMA rampup.
        if not opts.resume_blur:
            c.loss_kwargs.blur_init_sigma = 0 # Disable blur rampup.
            c.loss_kwargs.gpc_reg_fade_kimg = 0 # Disable swapping rampup

    if opts.resume_en is not None:
        c.resume_en = opts.resume_en


    if opts.resume_disc is not None:
        c.resume_disc = opts.resume_disc
    ...
    # Set target to be trained
    c.train_en = opts.train_en
    c.train_gen = opts.train_gen
    c.gan_depth = opts.gan_depth
    c.update_model = opts.update_model
