...
def generate_ffhq_samples(batch_size, G, all_label_paths, device):
    ...
    w_c = G.mapping(z=all_gen_z, c=origin_c, truncation_psi=0.8, truncation_cutoff=14)
    origin_img = G.synthesis(ws=w_c, c=origin_c, noise_mode='const')
    # w_c = G.mapping(z=all_gen_z, c=loss_c)
    loss_img = G.synthesis(ws=w_c, c=loss_c, noise_mode='const')
...
if __name__ == "__main__":
    root_dir = '/home/huangzixiong/dataset/classnerf/ShapeNet_GEN_DATASET/SRN_CARS_0.8/'
    ...
    with open('/home/huangzixiong/dataset/classnerf/srn/cars_train/label/labels.json', 'r') as file:
        all_label_paths = json.load(file)
    image_number = 62000
    device = torch.device('cuda', 0)
    with dnnlib.util.open_url('shapenetcars128-64.pkl') as f:
    ...
    for num in tqdm(range(image_number)):
        ...
        out_dir = os.path.join(root_dir, str(num).zfill(6))
        ...
        for name, data in zip([depth_name_1, depth_name_2], [output_data['origin_img_depth'][0].cpu().numpy(), output_data['loss_img_depth'][0].cpu().numpy()]):
            np.save(name, data)
    aggregate_data.aggregate_pose_label(root_dir)
    aggregate_data.aggregate_depth_images(root_dir)