    ...
    for step in tqdm(range(num_steps)):
        ...
        if step % image_log_step == 0:
            with torch.no_grad():
                ...
                PIL.Image.fromarray(vis_img[0].cpu().numpy(), 'RGB').save(f'./seed000.png')
        ...
        loss = dist + reg_loss * 1e5
        
        # if step % image_log_step == 0:
        #     with torch.no_grad():
        #         if use_wandb:
        #             global_config.training_step += 1
        #             wandb.log({f'loss': loss.detach().cpu()}, step=global_config.training_step)
        #             log_utils.log_image_from_w(w_opt.repeat([1, 14, 1]), G, w_name, camera_params)
        ...
        #             PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'./seed{100:04d}.png')

        if step % 50 == 0:
            torch.save(w_opt.repeat([1, 14, 1]).detach().cpu(), f"./w_save/{step}.pt")
...
@click.option('--json_path', 'json_path', help='Network pickle filename', required=True)
def main(network_pkl: str, img_path: str, json_path: str):
    ...
    intrinsics = 0
    for k in cam.keys():
        intrinsics = torch.tensor(cam[k]['intrinsics']).to(device)
        cam2world_pose = torch.tensor(cam[k]['pose']).to(device)

    image = Image.open(os.path.join(img_path, k)).convert('RGB')
    ...
    # np.save("hww.npy", camera_params.cpu().numpy())
    # image = np.array(PIL.Image.open(os.path.join(img_path, k)))
    # image = torch.from_numpy(image.transpose(2, 0, 1)).to(device).to(torch.float32) / 127.5 - 1 # HWC => CHW

    project(G=G, target=image, num_steps=450, camera_params=camera_params, device=device, w_name='20220810')
