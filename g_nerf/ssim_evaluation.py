...
def generate_talking_videos(
    ...
    if dataset == 'ffhq':
        ...
        with open(label_path, 'r') as f:
            labels = json.load(f)
    if dataset == 'afhqv2':
        all_paths = glob.glob(os.path.join(dataset_dir, "*.png"))
        all_paths.sort()
        eval_imgs = all_paths[4000:]
        with open(label_path, 'r') as f:
            labels = json.load(f)
    if dataset == 'celeba':
        l2_loss = torch.nn.MSELoss()
        all_paths = glob.glob(os.path.join(dataset_dir, "*.jpg"))
        all_paths.sort()
        all_depths = glob.glob("/data_14t/data_hzx/zs3nerf/CelebAMask-HQ/depth_maps/*_depth.jpg")
        all_depths.sort()
        eval_imgs = all_paths
        with open(label_path, 'r') as f:
            labels = json.load(f)
