...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(ws, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
...
class ResNeXt50(BaseNetwork):
    def __init__(self, num_gpus=1, out_dim=512):
        super().__init__()
        self.out_dim = out_dim
        # assert 2048 % out_dim == 0, "Out dim should be divisible by 2048"
        if num_gpus > 1:
            bn = nn.SyncBatchNorm
        else:
            bn = nn.BatchNorm2d
        self.model = ResNet(Bottleneck, [3, 4, 6, 3], groups=32, width_per_group=4, norm_layer=bn)
        self.avgpool = nn.AdaptiveAvgPool2d((2,2))
        self.fc = nn.Linear(2048*4, self.out_dim)
    ...
    def forward(self, input):
        net = self.forward_feature(input)
        net = self.fc(net)
...
if __name__ == '__main__':
    print("hello")
    encoder_id = ResNeXt50(out_dim=1024)
    input = torch.randn((3,3,128,128))
    print(encoder_id(input).shape)